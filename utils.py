## Ref - https://github.com/jessicayung/self-driving-car-nd/blob/master/p4-advanced-lane-lines/
...
def dir_threshold(image, sobel_kernel=5, thresh=(0, np.pi / 2)):
    ...
    output = np.zeros_like(abs_grad_dir)
    output[(abs_grad_dir >= thresh[0]) & (abs_grad_dir <= thresh[1])] = 1
    return output


def mag_thresh(img, sobel_kernel=11, mag_thresh=(30, 255)):
